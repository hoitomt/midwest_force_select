app.service('Player', function($q, $http) {
  ...
  this.fetchPlayer = function(playerId) {
    var deferred = $q.defer();
    $http.get(playerDatabaseUrlPlayers + playerId)
      .then(function(response){
        console.log(response.data);
        deferred.resolve(response.data);
      });
  ...
  this.uploadPlayerPhoto = function(pictureFile){
    var formData = new FormData();
    formData.append('player_photo', pictureFile);
    var deferred = $q.defer();
    $http({
      method: 'POST',
      url: playerDatabaseUrl + 'media/upload_photo',
      headers: {
        'Content-Type': undefined
      },
      data: formData,
    }).then(function(response){
      console.log("uploadPlayerPhoto promise", response);
      deferred.resolve(response.data);
    });
    return deferred.promise;
  }
  ...
  this.create = function(teamId, playerData) {
    console.log("create playerData: ", playerData);
    var deferred = $q.defer();
    jPlayerData = JSON.stringify({player: playerData});
  ...
  this.update = function(playerData) {
    console.log("update playerData: ", playerData);
    var deferred = $q.defer();
    jPlayerData = JSON.stringify({player: playerData});
